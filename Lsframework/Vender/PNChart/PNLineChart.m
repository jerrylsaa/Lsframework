...
@property (nonatomic ,assign) float moveValue;
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            if (chartData.inflexionPointStyle == PNLineChartPointStyleCircle) {
                ...
                BOOL yValueType = YES;
//                NSLog(@"###%f",_moveValue);
                if ([self.chartType isEqualToString:@"身高"]) {
                    
                    yValueType = yValue > 20;
                    
                }else if ([self.chartType isEqualToString:@"体重"]){
                    yValueType = yValue > 0;
                }
                ...
                if (yValueType == YES ) {
                    ...
                    if (i > 0 && _moveValue > _yFixedValueMin) {
    ...
    if (self.isShowCoordinateAxis) {
        ...
        if (self.showLabel) {
            ...
            for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
                int x = i * _xLabelWidth + _chartMarginLeft + _xLabelWidth / 2.0;
                point = CGPointMake(x, _chartMarginBottom + _chartCavanHeight);
