...
- (UILabel *)dayLabel {
    if (!_dayLabel) {
        _dayLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height)];
...
@property (nonatomic ,strong) NSMutableArray *dataSource;//当前月份已有记录日期
...
//查询数据库当前月份已有记录日期，记入数组，重载数据使用
- (void)setDate:(NSDate *)date {
    _date = date;
    NSDateFormatter *fomatter = [[NSDateFormatter alloc] init];
    [fomatter setDateFormat:@"yyyy"];
    NSString *year = [fomatter stringFromDate:date];
    [fomatter setDateFormat:@"MM"];
    NSString *month = [fomatter stringFromDate:date];
    NSArray *dateArray = [DateEntity findDateWithYear:year andMonth:month];
    [_dataSource removeAllObjects];
    for (DateEntity * entity in dateArray) {
        [_dataSource addObject:entity.day];
    }
    [self.collectionView reloadData];
    [ProgressUtil dismiss];
    if (_dataSource.count == 0) {
        [ProgressUtil showInfo:@"本月无健康日志记录"];
    }
}
...
//        NSLog(@"+ + + + + +  + + + +%@",dayStr);
        if ([_dataSource containsObject:dayStr]) {
            cell.backgroundView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"date_select"]];
        ...
        if (day == [[NSCalendar currentCalendar] component:NSCalendarUnitDay fromDate:self.date]) {
            NSDateFormatter *fomatter = [[NSDateFormatter alloc] init];
            [fomatter setDateFormat:@"yyyy"];
            NSString *yearStr = [fomatter stringFromDate:[NSDate date]];
            NSString *yearCurrent = [fomatter stringFromDate:self.date];
            [fomatter setDateFormat:@"MM"];
            NSString *monthStr = [fomatter stringFromDate:[NSDate date]];
            NSString *monthCurrent = [fomatter stringFromDate:self.date];
            if ([yearStr isEqualToString:yearCurrent] && [monthStr isEqualToString:monthCurrent]) {
