...
@interface COtherTypeTableViewCell ()
...
@implementation COtherTypeTableViewCell
...
-(void)setFavorite:(MyFavorite *)favorite
{
    if ([favorite isKindOfClass:[MyFavorite class]]) {
        ...
        _titleLb.text = favorite.NickName;
//        _contentLb.text = favorite.Content;
        _contentLb.attributedText = [UILabel getAttributeTextWithString:[NSString  stringWithFormat:@"%@",favorite.Content]];
        _timeLb.text = favorite.Time;
        _describeLb.text = favorite.Message;
        
        if (favorite.IsRead == YES) {
