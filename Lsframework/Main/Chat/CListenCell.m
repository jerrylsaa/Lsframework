...
-(void)setListen:(CConsultationBeListen *)listen
{
    if ([listen isKindOfClass:[CConsultationBeListen class]]) {
        ...
        _titleLb.text = listen.NickName;
//        _contentLb.text = listen.Content;
        _contentLb.attributedText = [UILabel getAttributeTextWithString:[NSString  stringWithFormat:@"%@",listen.Content]];
        _timeLb.text = listen.Time;
        _describeLb.text = listen.Message;
        
        if (listen.IsRead == YES) {
            _redDotImg.hidden = YES;
        }else{
            _redDotImg.hidden = NO;
        }
        
        _contentLb.width = [JMFoundation calLabelHeght:_contentLb];
