...
#import "PageView.h"
@interface SNAP_IVViewController ()<XLLBPresenterDelegate,PageViewDelegate>
@property (nonatomic, strong)PageView *pageView;
@property (nonatomic,strong) XLLBPresenter *presenter;

@end

@implementation SNAP_IVViewController
...
- (void)setupView{
    ...
    self.presenter.tableName =@"SNAP_IV";
    [self setupPage];
...
- (void)onCompletion:(BOOL)success info:(NSString *)messsage{
    if(success){
        SNAP_IVCSViewController *ceshiVC =[SNAP_IVCSViewController new];
