...
@interface GNSHPGViewController ()<XLLBPresenterDelegate,PageViewDelegate>
...
@implementation GNSHPGViewController
...
- (void)setupView{
    ...
    self.presenter.tableName =@"GNSHPG";
    [self setupPage];
...
- (void)onCompletion:(BOOL)success info:(NSString *)messsage{
    if(success){
        GNSHPGCSViewController *ceshiVC =[GNSHPGCSViewController new];
