...
- (void)setMyListen:(MyListen *)myListen{
    _myListen = myListen;
    //赋值
    _titleLabel.text = _myListen.ConsultationContent;
    [_headImageView sd_setImageWithURL:[NSURL URLWithString:_myListen.ImageUrl] placeholderImage:[UIImage imageNamed:@"doctor_default"]];
    _nameLabel.text = _myListen.DoctorName;
    _postLabel.text = _myListen.Introduce;
//  NSString* myListenTime = [NSDate getDateCompare:_myListen.CreateTime];
//    _timeLabel.text =myListenTime;
    _timeLabel.text =_myListen.CreateTime;
    _listenLabel.text = [NSString stringWithFormat:@"听过  %@",_myListen.HearCount];
    //布局
    _titleLabel.sd_layout.leftSpaceToView(self.contentView,10).topSpaceToView(self.contentView,15).rightSpaceToView(self.contentView,10);
    ...
    _listenLabel.sd_layout.rightSpaceToView(self.contentView,10).topEqualToView(_timeLabel).widthIs(100).heightIs(15);
