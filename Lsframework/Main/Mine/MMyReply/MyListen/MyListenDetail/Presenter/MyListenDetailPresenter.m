...
@interface MyListenDetailPresenter ()<ZHAVRecoderDelegate>
...
@implementation MyListenDetailPresenter
...
- (void)GetExpertConsultationByUUID:(NSInteger)uuid finish:(LoadHandler)block{
    ...
    [[FPNetwork POST:@"GetExpertConsultation" withParams:@{@"userid":@(kCurrentUser.userId),@"UUID":@(uuid)}] addCompleteHandler:^(FPResponse *response) {
        if (response.success == YES) {
            ...
            ws.myListen = array.firstObject;
            ws.myListen.ConsultationID = [NSNumber numberWithInteger:uuid];
            ws.url = ws.myListen.VoiceUrl;
            block(YES,nil);
        }else{
            block(NO,nil);
        }
