...
@interface FamilyDoctorApplyCell (){
    UIView* _container;
    UILabel* _time;
    UILabel* _patitent;
    UILabel* _state;//状态
...
@implementation FamilyDoctorApplyCell
...
- (void)setDoctor:(MineApplyFamilyDoctorEntity *)doctor{
    ...
    //患者
    NSString* patientStr = [NSString stringWithFormat:@"患者：%@",doctor.fdBabyName];
...
//    }
    ...
    //套餐价格
    NSString* price = [NSString stringWithFormat:@"付费套餐：%ld元/年",doctor.fdprice];
    if(doctor.fdprice == 0){
        price = @"付费套餐：";
    }
