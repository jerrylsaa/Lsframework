...
@interface HEAInfoViewController ()<UITableViewDelegate,UITableViewDataSource,HEAInfoPresenterDelegate,UIActionSheetDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate,ECouponViewControllerDelegate,UITextViewDelegate,PhotoBrowerDelegate,UIAlertViewDelegate,PraiseDelegate>{
    CGPoint pointAdd;
    CGPoint pointReduce;
    UIView* headerView;
    CGFloat  downlbFont;
...
@implementation HEAInfoViewController
...
- (void)loadData{
    ...
    [self.presenter loadCommonExpertConsultaionData:[_hospitalType isEqualToString:@"MyBindHospital"]?self.expertEntity.HOSPITAL_ID:self.expertEntity.doctorID status:-1];
...
#pragma mark----判断当前医生是否是点击进入的医生
   ...
   //提问次数
    NSString* times = (self.expertEntity.hearCount != 0?[NSString stringWithFormat:@"已有%@次提问",self.expertEntity.consultCount]: @"");
...
//    [_table registerClass:[HEAParentQuestionTableViewCell class] forCellReuseIdentifier:@"cell"];
    ...
    UIImageView *lineline =[UIImageView new];
    lineline.backgroundColor =UIColorFromRGB(0x999999);
    [footerView addSubview:lineline];
    lineline.sd_layout.topSpaceToView(footerView,10).leftSpaceToView(footerView,15).rightSpaceToView(footerView,15).heightIs(1);
    
    UIImageView *huanyiboIV =[UIImageView new];
    huanyiboIV.image =[UIImage imageNamed:@"huanyibo"];
    [footerView addSubview:huanyiboIV];
    huanyiboIV.sd_layout.leftSpaceToView(footerView,kScreenWidth/2.0f-51.5f).topSpaceToView(lineline,5).widthIs(23).heightIs(23);
    UILabel *hybLabel =[UILabel new];
    hybLabel.text =@"换一波";
    hybLabel.font =[UIFont boldSystemFontOfSize:15.0f];
    hybLabel.textColor =UIColorFromRGB(0x333333);
    [footerView addSubview:hybLabel];
    hybLabel.sd_layout.leftSpaceToView(huanyiboIV,5.0f).centerYEqualToView(huanyiboIV).widthIs(80).heightIs(23);
    ...
    _table.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        [ws.presenter loadCommonExpertConsultaionData:[_hospitalType isEqualToString:@"MyBindHospital"]?ws.expertEntity.HOSPITAL_ID:ws.expertEntity.doctorID status:-1];
    }];
    
//    _table.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
//        ws.table.userInteractionEnabled = NO;
//        [ws.presenter loadHYBExpertConsultaionData:[_hospitalType isEqualToString:@"MyBindHospital"]?ws.expertEntity.HOSPITAL_ID:ws.expertEntity.doctorID status:-1];
//    }];
...
//            photoImage.layer.cornerRadius= 8;
            ...
            if (i>4) {
                ...
                photoImage.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageWidth+kImageTopspace).leftSpaceToView(_HaveImageView,(i-5)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
                
                
            }else{
                
                photoImage.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,(i-1)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
...
//    _firstCommentLabel.text =@"ashgdhjasdhwqhdiqwuhdiuqw";
    ...
    _HaveImageView.sd_layout.topSpaceToView(self.textView,15).leftEqualToView(self.textView).rightEqualToView(self.textView).heightIs(kImageWidth+15);
    ...
    self.headerTitlebgView.sd_layout.topSpaceToView(self.commitbt,15).leftSpaceToView(headerView,0).rightSpaceToView(headerView,0).heightIs(50);
    self.titleTips.sd_layout.topSpaceToView(self.headerTitlebgView,15).heightIs(20).leftSpaceToView(self.headerTitlebgView,10).widthIs(150);
    ...
    [lookMoreBtn setTitleColor:UIColorFromRGB(0x666666) forState:UIControlStateNormal];
    lookMoreBtn.titleLabel.font =[UIFont systemFontOfSize:16.0f];
    lookMoreBtn.titleLabel.textAlignment =NSTextAlignmentRight;
    [self.headerTitlebgView addSubview:lookMoreBtn];
    lookMoreBtn.sd_layout.topSpaceToView(self.headerTitlebgView,15).heightIs(20).rightSpaceToView(self.headerTitlebgView,0).widthIs(100);
...
//        cell.isFree =YES;
    ...
    [cell clickAudionButtonOnCompletion:^(UIButton *bt) {
        ...
        if(question.isListen|weakCell.isFree|ws.isDoctor){
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"---is---%@",question.consultationContent);
    ...
    if (success) {
        if ([self.wxPayType isEqualToString:@"questionBiz"]) {
            ...
            if (_uploadImageArr.count!=0) {
                ...
                _uploadImageArr =nil;
                _photoArray =nil;
                self.HaveImageView.sd_layout.topSpaceToView(self.textView,15).leftEqualToView(self.textView).rightEqualToView(self.textView).heightIs(kImageWidth+15);
//                [self.TopView  updateLayout];
//                [self.MidView  updateLayout];
                [headerView  updateLayout];
    ...
    if (success) {
        ...
        if (self.expertEntity.price > 0) {
            ...
            if ([_presenter.Couponcount  integerValue] >0 ) {
                //咨询次数大于0
                self.pricenLabel.hidden = YES;
                self.pricenLabel.sd_layout.heightIs(0);
                [_commitbt setTitle:@"免费义诊" forState:UIControlStateNormal];
                [_commitbt  setImage:nil  forState:UIControlStateNormal];
                [_commitbt setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
                self.commitbt.sd_layout.topSpaceToView(self.moneyTips,15).heightIs(100/2).centerXEqualToView(headerView).widthIs(100);
            }else{
                //写好了、
                self.pricenLabel.hidden = NO;
                self.pricenLabel.sd_layout.topSpaceToView(self.moneyTips,15).heightIs(15).leftSpaceToView(headerView,0).rightSpaceToView(headerView,0);
                if([_textView.text trimming].length >= 20){
                    if(self.expertEntity.price==0){
                        [_commitbt  setImage:[UIImage  imageNamed:@"HEInfoCommit_FreeBtn"] forState:UIControlStateNormal];
        ...
        [self.pricenLabel  updateLayout];
        [_commitbt  updateLayout];
        [headerView  updateLayout];
        [headerView layoutSubviews];
        _table.tableHeaderView = headerView;
...
- (void)textDidChange{
    ...
    if([_textView.text trimming].length >= 20){
        if(self.expertEntity.price==0){
            ...
            if ([_presenter.Couponcount integerValue] >0) {
        [_commitbt  setImage:[UIImage  imageNamed:@"HEInfoCommit_FreeClinc"] forState:UIControlStateNormal];
            }else{
                [_commitbt  setImage:[UIImage  imageNamed:@"HEInfoCommit_CMBtn"] forState:UIControlStateNormal];
            }
        }
...
- (void)lookMoreAction{
    NSLog(@"更多");
    SecondHEAInfoViewController *vc =[SecondHEAInfoViewController new];
...
- (void)deleteAction:(UIButton*) bt{
    ...
    if(imageView.tag == 3001){
        ...
        if(self.uploadImageArr.count == 1){
        ...
        }else if(self.uploadImageArr.count == 4){
            [self.uploadImageArr removeObjectAtIndex:0];
            [self.photoArray removeObjectAtIndex:0];
            _addImageBtn.hidden = NO;
            ...
            UIImageView* ImageView2 = [self.upImageViewArr objectAtIndex:1];
            ImageView2.image = [self.uploadImageArr objectAtIndex:1];
            
            UIImageView* ImageView3 = [self.upImageViewArr objectAtIndex:2];
            ImageView3.image = [self.uploadImageArr objectAtIndex:2];
        }else if(self.uploadImageArr.count == 5){
            [self.uploadImageArr removeObjectAtIndex:0];
            [self.photoArray removeObjectAtIndex:0];
            _addImageBtn.hidden = NO;
            ...
            UIImageView* ImageView4 = [self.upImageViewArr objectAtIndex:3];
            ImageView4.image = [self.uploadImageArr objectAtIndex:3];
        }else if(self.uploadImageArr.count == 6){
            [self.uploadImageArr removeObjectAtIndex:0];
            [self.photoArray removeObjectAtIndex:0];
            _addImageBtn.hidden = NO;
            ...
            UIImageView* ImageView5 = [self.upImageViewArr objectAtIndex:4];
            ImageView5.image = [self.uploadImageArr objectAtIndex:4];
            
        }
        
        for (int i = (int )_uploadImageArr.count; i<6; i++) {
            if (_upImageViewArr[i]) {
                UIImageView *photoImagees =(UIImageView *)(_upImageViewArr[i]);
                photoImagees.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,0).widthIs(0).heightIs(50);
        ...
        if(self.uploadImageArr.count == 2){
            ...
            imageView.image = nil;
            imageView.image = [self.uploadImageArr objectAtIndex:1];
        ...
        for (int i = (int )_uploadImageArr.count; i<6; i++) {
            if (_upImageViewArr[i]) {
                UIImageView *photoImagees =(UIImageView *)(_upImageViewArr[i]);
                photoImagees.sd_layout.centerYEqualToView(_HaveImageView).leftSpaceToView(_HaveImageView,0).widthIs(0).heightIs(50);
                photoImagees.hidden = YES;
            }
        ...
        //第3张图片
        if(self.uploadImageArr.count == 3){
            ...
            [self.uploadImageArr removeObjectAtIndex:2];
            [self.photoArray removeObjectAtIndex:2];
            
            
            imageView.image = nil;
            imageView.image = [self.uploadImageArr objectAtIndex:2];
            ...
            UIImageView* ImageView3 = [self.upImageViewArr objectAtIndex:3];
            ImageView3.image = [self.uploadImageArr objectAtIndex:3];
            ...
            UIImageView* ImageView4 = [self.upImageViewArr objectAtIndex:4];
            ImageView4.image = [self.uploadImageArr objectAtIndex:4];
    ...
    }else if(imageView.tag == 3004){
        //第4张图片
        if(self.uploadImageArr.count == 4){
            ...
            [self.uploadImageArr removeObjectAtIndex:3];
            [self.photoArray removeObjectAtIndex:3];
            
            
            imageView.image = nil;
            imageView.image = [self.uploadImageArr objectAtIndex:3];
            ...
            UIImageView* ImageView3 = [self.upImageViewArr objectAtIndex:4];
            ImageView3.image = [self.uploadImageArr objectAtIndex:4];
    ...
    }else if(imageView.tag == 3005){
        //第5张图片
        if(self.uploadImageArr.count == 5){
            _addImageBtn.hidden = NO;
            
            [self.uploadImageArr removeObjectAtIndex:4];
            [self.photoArray removeObjectAtIndex:4];
            imageView.image = nil;
            ...
            imageView.image = nil;
            imageView.image = [self.uploadImageArr objectAtIndex:4];
    ...
    }else if(imageView.tag == 3006){
        //第6张图片
        if(self.uploadImageArr.count == 6){
            _addImageBtn.hidden = NO;
            
            [self.uploadImageArr removeObjectAtIndex:5];
            [self.photoArray removeObjectAtIndex:5];
            imageView.image = nil;
        }
    ...
    if (_uploadImageArr.count>=4) {
        
        self.HaveImageView.sd_layout.topSpaceToView(self.textView,15).leftEqualToView(self.textView).rightEqualToView(self.textView).heightIs(kImageWidth+15+kImageTopspace+kImageWidth);
    ...
    if (self.uploadImageArr.count==1){
        ...
        self.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,kImageXspace+kImageWidth).widthIs(kImageWidth).heightIs(kImageWidth);
    }else if (self.uploadImageArr.count==2){
        
        self.addImageBtn.hidden = NO;
        self.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,2*(kImageXspace+kImageWidth)).widthIs(kImageWidth).heightIs(kImageWidth);
        
        
    }else if (self.uploadImageArr.count==3){
        self.addImageBtn.hidden = NO;
        self.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,3*(kImageXspace+kImageWidth)).widthIs(kImageWidth).heightIs(kImageWidth);
    }else if (self.uploadImageArr.count==4){
        self.addImageBtn.hidden = NO;
        self.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageTopspace+kImageWidth).leftSpaceToView(_HaveImageView,0).widthIs(kImageWidth).heightIs(kImageWidth);
    }else if (self.uploadImageArr.count==5){
        self.addImageBtn.hidden = NO;
        self.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageTopspace+kImageWidth).leftSpaceToView(_HaveImageView,kImageXspace+kImageWidth).widthIs(kImageWidth).heightIs(kImageWidth);
    }else {
        self.addImageBtn.hidden = YES;
    }
    
    
    [self.addImageBtn  updateLayout];
...
- (void)footerTapAction{
    ...
    [_presenter loadHYBExpertConsultaionData:[_hospitalType isEqualToString:@"MyBindHospital"]?_expertEntity.HOSPITAL_ID:_expertEntity.doctorID status:-1];
}
...
-(UILabel *)pricenLabel{
    if(!_pricenLabel){
        ...
        _pricenLabel.textColor = UIColorFromRGB(0x61d8d3);
...
//    _uploadImageArr =nil;
    ...
    if(_uploadImageArr && _uploadImageArr.count != 0){
        if(_uploadImageArr.count == 1){
            ...
            count = 3;
        }else if(_uploadImageArr.count == 4){
            count = 2;
        }else if(_uploadImageArr.count == 5){
            count = 1;
        }else if(_uploadImageArr.count == 6){
            count = 0;
        }
    }else{
        count = 6;
            ...
            runOnMainThread(^{
                    ...
                    if (_uploadImageArr.count!=6) {
                        ...
                        for (int i = (int )_uploadImageArr.count; i<6; i++) {
                            if (_upImageViewArr[i]) {
                                ...
                                if (i<5) {
                                    ...
                                    photoImagees.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,(_uploadImageArr.count)*(kImageXspace+kImageWidth)).widthIs(0).heightIs(kImageWidth);
                                    
                                }else{
                                    photoImagees.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageWidth+kImageTopspace).leftSpaceToView(_HaveImageView,(_uploadImageArr.count-5)*(kImageXspace+kImageWidth)).widthIs(0).heightIs(kImageWidth);
                        ...
                        for (int i =1; i <_uploadImageArr.count+1; i++) {
                            if (_upImageViewArr[i-1]) {
                                ...
                                if (i<5) {
                                    photoImageetsss.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,(i-1)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
                                }else{
                                    photoImageetsss.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageWidth+kImageTopspace).leftSpaceToView(_HaveImageView,(i-5)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
                        ...
                        for (int i =1; i<=_uploadImageArr.count; i++) {
                            if (_upImageViewArr[i-1]) {
                                ...
                                if (i<5) {
                                    photoImagee.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,(i-1)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
                                }else{
                                    
                                    photoImagee.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageWidth+kImageTopspace).leftSpaceToView(_HaveImageView,(i-5)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (_uploadImageArr.count>=4) {
                        ...
                        ws.HaveImageView.sd_layout.topSpaceToView(self.textView,15).leftEqualToView(self.textView).rightEqualToView(self.textView).heightIs(kImageWidth+15+kImageTopspace+kImageWidth);
                        
                    }else{
                        
                        ws.HaveImageView.sd_layout.topSpaceToView(self.textView,15).leftEqualToView(self.textView).rightEqualToView(self.textView).heightIs(kImageWidth+15);
                    }
                    
                    if (self.uploadImageArr.count==1){
                        ws.addImageBtn.hidden = NO;
                        ws.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,kImageXspace+kImageWidth).widthIs(kImageWidth).heightIs(kImageWidth);
                    }else if (self.uploadImageArr.count==2){
                        
                        ws.addImageBtn.hidden = NO;
                        ws.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,2*(kImageXspace+kImageWidth)).widthIs(kImageWidth).heightIs(kImageWidth);
                        
                        
                    }else if (self.uploadImageArr.count==3){
                        ws.addImageBtn.hidden = NO;
                        ws.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,3*(kImageXspace+kImageWidth)).widthIs(kImageWidth).heightIs(kImageWidth);
                    }else if (self.uploadImageArr.count==4){
                        ws.addImageBtn.hidden = NO;
                        ws.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageTopspace+kImageWidth).leftSpaceToView(_HaveImageView,0).widthIs(kImageWidth).heightIs(kImageWidth);
                    }else if (self.uploadImageArr.count==5){
                        ws.addImageBtn.hidden = NO;
                        ws.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageTopspace+kImageWidth).leftSpaceToView(_HaveImageView,kImageXspace+kImageWidth).widthIs(kImageWidth).heightIs(kImageWidth);
                    }else {
                        ws.addImageBtn.hidden = YES;
                    }

                    [ws.addImageBtn  updateLayout];
//                    [ws.TopView  updateLayout];
//                    [ws.MidView  updateLayout];
                    [headerView  updateLayout];
                    [headerView layoutSubviews];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
        ...
        if (_uploadImageArr.count!=6) {
            ...
            for (int i =(int )_uploadImageArr.count; i<6; i++) {
                if (_upImageViewArr[i]) {
                    UIImageView *photoImagees =(UIImageView *)(_upImageViewArr[i]);
                    if (i<5) {
                        photoImagees.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,((_uploadImageArr.count)*(kImageXspace+kImageWidth))).widthIs(0).heightIs(0);
                    }else{
                        
                        photoImagees.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageTopspace+kImageTopspace).leftSpaceToView(_HaveImageView,((_uploadImageArr.count-5)*(kImageXspace+kImageWidth))).widthIs(0).heightIs(0);
            ...
            for (int i =1; i <_uploadImageArr.count+1; i++) {
                if (_upImageViewArr[i-1]) {
                    ...
                    if (i<5) {
                        photoImageesss.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,(i-1)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
                    }else{
                        photoImageesss.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageTopspace+kImageWidth).leftSpaceToView(_HaveImageView,(i-5)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
            ...
            for (int i =1; i<=_uploadImageArr.count; i++) {
                if (_upImageViewArr[i-1]) {
                    ...
                    if (i<5) {
                        ...
                        photoImagee.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace+kImageTopspace+kImageWidth).leftSpaceToView(_HaveImageView,(i-5)*(kImageWidth+kImageXspace)).widthIs(kImageWidth).heightIs(kImageWidth);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSLog(@"图片数量：%d", _uploadImageArr.count);
        if (self.uploadImageArr.count==1){
            ws.addImageBtn.hidden = NO;
            ws.addImageBtn.sd_layout.topSpaceToView(_HaveImageView,kImageTopspace).leftSpaceToView(_HaveImageView,kImageXspace+kImageWidth).widthIs(kImageWidth).heightIs(kImageWidth);
        }else if (self.uploadImageArr.count==2){
