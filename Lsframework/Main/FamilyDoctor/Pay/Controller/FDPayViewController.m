...
@interface FDPayViewController ()<UITableViewDelegate,UITableViewDataSource>{
    UITableView* _table;
}
...
@implementation FDPayViewController
...
-(void)setupView{
    ...
    self.sectionTitle=@[@"订 单 号:",@"套餐名称:",@"优 惠 券:",@"支付方式:",@"应付金额:"];
    ...
    commit.sd_layout.spaceToSuperView(UIEdgeInsetsMake(85, 25, 10, 25));
}
...
//    [self.navigationController pushViewController:[ACSuccessPageViewController new] animated:YES];
    ...
    [self.navigationController pushViewController:[FDMainViewController new] animated:YES];
