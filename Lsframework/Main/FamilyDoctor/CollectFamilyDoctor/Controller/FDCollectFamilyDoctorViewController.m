//
//  FDCollectFamilyDoctorViewController.m
//  FamilyPlatForm
//
...
@interface FDCollectFamilyDoctorViewController ()<UITableViewDataSource,UITableViewDelegate,FDCollectFamilyDoctorPresenterDelegate,JHCustomMenuDelegate>{
    UITableView* _table;
...
@property(nonatomic,retain) FDCollectFamilyDoctorPresenter* presenter;
...
@implementation FDCollectFamilyDoctorViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.presenter = [FDCollectFamilyDoctorPresenter new];
...
//        [self.presenter loadDoctorData:self.province City:self.city];
        [self.presenter loadDoctorData:self.city];
    }else{
        //定位失败后，重新获取城市
        WS(ws);
        [ProgressUtil show];
...
//                [ws.presenter loadDoctorData:province City:city];
                [ws.presenter loadDoctorData:city];
            }else{
...
- (void)setupTableView{
    ...
    [_table registerClass:[CollectionDoctorTableViewCell class] forCellReuseIdentifier:@"cell"];
}
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    CollectionDoctorTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    
    CollectDoctor* doctor = self.dataSource[indexPath.row];
    cell.doctor = doctor;
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ACDoctorDetailViewController* doctorInfo = [ACDoctorDetailViewController new];
    doctorInfo.doctorId = @3.0;//测试
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [_table cellHeightForIndexPath:indexPath model:doctor keyPath:@"doctor" cellClass:[CollectionDoctorTableViewCell class] contentViewWidth:[self cellContentViewWith]];
}
