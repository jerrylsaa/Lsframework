//
//  CollectionDoctorTableViewCell.m
//  FamilyPlatForm
//
...
@interface CollectionDoctorTableViewCell (){
...
@implementation CollectionDoctorTableViewCell
...
-(void)setDoctor:(CollectDoctor *)doctor{
    _doctor = doctor;
    //头像
    ...
    //职称
    NSString* text = doctor.dictionaryName;
    _postLabel.text=text;
    //星级
    [_start setScore:[doctor.starNum floatValue]/10.0f withAnimation:YES];
    //姓名
    ...
    //患者
    _patientLabel.text = [NSString stringWithFormat:@"患者：%@例",doctor.patientNum ];
    //随访
    _followLabel.text = [NSString stringWithFormat:@"随访：%@例",doctor.followUp];
...
//    _onlineLabel.hidden = isHidden;
    ...
    //签约患者
    _signPatient.text = [NSString stringWithFormat:@"签约患者：%@例",doctor.familyDoctor];
